...
void sr_handle_ip_packet(struct sr_instance *sr,
  ...
  if (eHdr->ether_type == htons(ethertype_ip)) { 
    ...
    if (ip_cksum(ipHdr, sizeof(sr_ip_hdr_t)) == ipHdr->ip_sum){ 
      /* Query routing table */
      struct sr_if* myIface = sr_get_interface_given_ip(sr, ipHdr->ip_dst); /* Get the interface */
      ...
      if (ipHdr->ip_p == ip_protocol_icmp ){ /* EXTRA CHECK*/
        int cumulative_sz = sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t);
        sr_icmp_hdr_t *icmpHdr = (sr_icmp_hdr_t *) (packet + cumulative_sz);
        /*printf("**** -> ICMP type: %d\n", icmpHdr->icmp_type);        
        printf("**** -> ICMP checksum: %d == %d\n", (icmp_cksum(icmpHdr, len - cumulative_sz)), icmpHdr->icmp_sum);*/
